...
class _MilkEntryEditScreenState extends State<MilkEntryEditScreen> {
  ...
  late List<IncomeModel> _entries; // 0: Today, 1: Yesterday, 2: Day before
  ...
  Future<void> _loadIncomeEntries() async {
    ...
    _entries = await _incomeService.getIncomeEntriesForDateRange(
      startDate: startDate,
      endDate: endDate,
      userId: _userId!,
  ...
  Widget _entryTile(int index) {
    ...
    final milk = e.liters.toString();
    final snf = e.snf.toString();
    final fat = e.fat.toString();
    final cost = e.costPerLiter.toString();
    final incomeId = e.id;
    return Padding(
